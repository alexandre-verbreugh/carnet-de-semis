...
class SpeciesController extends AbstractController
{
    ...
    public function index(Request $request, SpeciesRepository $speciesRepository): Response
    {
        ...
        return $this->render('species/index.html.twig', [
            'especes' => $speciesRepository->search($terme, $categorie, $moisCourant),
            'terme' => $terme,
            'categorie' => $categorie,
    ...
    public function edit(Request $request, Species $espece, EntityManagerInterface $entityManager): Response
    {
        $form = $this->createForm(SpeciesType::class, $espece);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            // Modifier une fiche du catalogue livre la rend personnelle, sans
            // quoi le prochain import ecraserait la correction.
            $espece->setIsCustom(true);
            $entityManager->flush();
