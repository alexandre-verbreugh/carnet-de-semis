 ...
 */
final class Version20260815162137 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE observation (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, observed_at DATE NOT NULL, type VARCHAR(255) NOT NULL, note CLOB DEFAULT NULL, height_cm INTEGER DEFAULT NULL, leaf_count INTEGER DEFAULT NULL, germinated_count INTEGER DEFAULT NULL, harvest_grams INTEGER DEFAULT NULL, created_at DATETIME NOT NULL, sowing_id INTEGER DEFAULT NULL, plot_id INTEGER NOT NULL, owner_id INTEGER NOT NULL, CONSTRAINT FK_C576DBE0392BF32A FOREIGN KEY (sowing_id) REFERENCES sowing (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_C576DBE0680D0B01 FOREIGN KEY (plot_id) REFERENCES plot (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_C576DBE07E3C61F9 FOREIGN KEY (owner_id) REFERENCES user (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('CREATE INDEX IDX_C576DBE0392BF32A ON observation (sowing_id)');
        $this->addSql('CREATE INDEX IDX_C576DBE0680D0B01 ON observation (plot_id)');
        $this->addSql('CREATE INDEX IDX_C576DBE07E3C61F9 ON observation (owner_id)');
        ...
        $this->addSql('CREATE TABLE plot (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(100) NOT NULL, type VARCHAR(255) NOT NULL, shelter VARCHAR(255) NOT NULL, location VARCHAR(150) DEFAULT NULL, length_cm INTEGER DEFAULT NULL, width_cm INTEGER DEFAULT NULL, depth_cm INTEGER DEFAULT NULL, substrate_components CLOB NOT NULL, top_layer VARCHAR(255) DEFAULT NULL, substrate_note CLOB DEFAULT NULL, exposure VARCHAR(255) DEFAULT NULL, has_drainage BOOLEAN NOT NULL, filled_at DATE DEFAULT NULL, is_archived BOOLEAN NOT NULL, owner_id INTEGER NOT NULL, CONSTRAINT FK_BEBB8F897E3C61F9 FOREIGN KEY (owner_id) REFERENCES user (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('CREATE INDEX IDX_BEBB8F897E3C61F9 ON plot (owner_id)');
        $this->addSql('CREATE TABLE seed_lot (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, brand VARCHAR(100) DEFAULT NULL, lot_ref VARCHAR(60) DEFAULT NULL, purchased_at DATE DEFAULT NULL, expires_at DATE DEFAULT NULL, initial_seed_count INTEGER DEFAULT NULL, remaining_seed_count INTEGER DEFAULT NULL, notes CLOB DEFAULT NULL, species_id INTEGER NOT NULL, owner_id INTEGER NOT NULL, CONSTRAINT FK_CD568AC1B2A1D860 FOREIGN KEY (species_id) REFERENCES species (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_CD568AC17E3C61F9 FOREIGN KEY (owner_id) REFERENCES user (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('CREATE INDEX IDX_CD568AC1B2A1D860 ON seed_lot (species_id)');
        $this->addSql('CREATE INDEX IDX_CD568AC17E3C61F9 ON seed_lot (owner_id)');
        $this->addSql('CREATE TABLE sowing (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, sown_at DATE NOT NULL, seed_count INTEGER DEFAULT NULL, method VARCHAR(255) NOT NULL, depth_mm INTEGER DEFAULT NULL, status VARCHAR(255) NOT NULL, germinated_at DATE DEFAULT NULL, germinated_count INTEGER DEFAULT NULL, ended_at DATE DEFAULT NULL, failure_reason CLOB DEFAULT NULL, notes CLOB DEFAULT NULL, plot_id INTEGER NOT NULL, owner_id INTEGER NOT NULL, species_id INTEGER NOT NULL, seed_lot_id INTEGER DEFAULT NULL, CONSTRAINT FK_BE50E408680D0B01 FOREIGN KEY (plot_id) REFERENCES plot (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_BE50E4087E3C61F9 FOREIGN KEY (owner_id) REFERENCES user (id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_BE50E408B2A1D860 FOREIGN KEY (species_id) REFERENCES species (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_BE50E408B548BD8A FOREIGN KEY (seed_lot_id) REFERENCES seed_lot (id) ON DELETE SET NULL NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('CREATE INDEX IDX_BE50E408680D0B01 ON sowing (plot_id)');
        $this->addSql('CREATE INDEX IDX_BE50E4087E3C61F9 ON sowing (owner_id)');
